...
EXTERNAL_IPS = [
    "45.33.32.156", "185.220.101.5", "94.102.49.190",
    "198.51.100.42", "203.0.113.17", "91.108.56.12",
    "104.244.42.1", "51.15.0.1", "77.109.139.87",
]

HOSTNAMES = [f"WORKSTATION-{n:03d}" for n in range(1, 21)] + \
...
def make_login_failure(dt: datetime, src_ip: str | None = None) -> dict:
    return {
        "timestamp": _ts(dt),
        "event_type": "authentication",
...
def make_process_event(dt: datetime, process: str | None = None) -> dict:
...
def make_file_event(dt: datetime, file_path: str | None = None) -> dict:
    return {
        "timestamp": _ts(dt),
...
def make_privilege_event(dt: datetime, privilege: str | None = None) -> dict:
    return {
        "timestamp": _ts(dt),
...
def scenario_brute_force(start: datetime, n: int) -> list[dict]:
    """Simulate brute force: bursts of failures from attacker IP, then a success."""
    events: list[dict] = []
...
def scenario_privilege_escalation(start: datetime, n: int) -> list[dict]:
    """Simulate privilege escalation: normal login, then sensitive privilege use."""
    events: list[dict] = []
    user = random.choice(USERS)
...
def scenario_lateral_movement(start: datetime, n: int) -> list[dict]:
    """Simulate lateral movement: PSExec / WMI calls across multiple hosts."""
    events: list[dict] = []
...
def scenario_data_exfiltration(start: datetime, n: int) -> list[dict]:
    """Simulate exfiltration: large outbound transfers to external IPs."""
    events: list[dict] = []
    src_ip = random.choice(INTERNAL_IPS)
...
def scenario_malware(start: datetime, n: int) -> list[dict]:
    """Simulate malware execution and C2 beaconing."""
    events: list[dict] = []
    ...
    for i in range(n):
        ...
        if i % 4 == 0:
            ...
            ev = make_network_event(dt, dest_ip=attacker_c2, bytes_out=random.randint(500, 5_000))
